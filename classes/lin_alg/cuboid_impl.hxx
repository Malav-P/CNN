...
Cuboid<T> &Cuboid<T>::operator+=(const Cuboid<T> &other)
{
    ...
    // do += operation
    for (size_t i = 0; i < _rows ; i++){ for (size_t j = 0; j < _cols ; j++){ for (size_t k = 0; k < _depth ; k++){
...
template<typename T>
const T& Cuboid<T>::operator()(size_t i, size_t j, size_t k) const { return _data[k*(_cols*_rows) + (i * _cols + j)];}
//! ---------------------------------------------------------------------------------------
...
T Cuboid<T>::partial_dot(const Cuboid<T>& other, Dims3 p)
{
    ...
    for (size_t i = 0; i < other._rows ; i++) { for (size_t j = 0; j< other._cols ; j++){ for (size_t k = 0; k< other._depth; k++){
...
{
    // initialize return variable
    Vector<T> obj(_rows * _cols * _depth);
...
void Cuboid<T>::keep(Dims3 *indices)
{
    ...
    for (size_t i = 0; i<_rows; i++) {for (size_t j = 0; j < _cols ; j++) { for (size_t k = 0 ; k < _cols ; k++){
            candidate = {i,j, k};
    ...
    for (size_t k = 0; k<_depth; k++) {
        ...
        for (size_t new_i = 0; new_i < _rows; new_i++) {
            for (size_t new_j = 0; new_j < _cols; new_j++) {
                _data[(k*_cols*_rows) + (new_i * _cols + new_j)] = tmp[(k*_cols*_rows) + ((_cols - 1 - new_j) * _rows) + new_i];
            }
        }
    }
