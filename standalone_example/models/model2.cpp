...
int main(int argc, char* argv[])
{
    ...
    model.Add<MaxPooling>(   model.get_outshape(4).width   // input width
            , model.get_outshape(4).height  // input height
            , model.get_outshape(4).depth   // in maps
            , 2  // filter width
            , 2  // filter height
            , 2  // horizontal stride length
            , 2  // vertical stride length
    ...
    model.Add<RelU>(    model.get_outshape(6).width,    // input width
                        model.get_outshape(6).height,   // input height
                        model.get_outshape(6).depth,    // input depth
                        0.1                             // leaky RelU parameter
    );

    model.Add<Linear>(   model.get_outshape(7).depth
                         *model.get_outshape(7).width
                         *model.get_outshape(7).height  // input size
            , 10                           // output size
    );


    model.Add<Softmax>(model.get_outshape(8).width * model.get_outshape(8).height * model.get_outshape(8).depth // input size
    );
